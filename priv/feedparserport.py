...
feedparser.USER_AGENT = (
    'Mozilla/5.0 AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2638.0 '
    'Safari/537.36 Draft/Feederer'
)

all_tags = [
    'author', 'author_detail', 'base', 'bozo', 'bozo_exception', 'cloud',
    'comments', 'content', 'contributors', 'created', 'created_parsed',
    'description', 'docs', 'domain', 'email', 'enclosures', 'encoding',
    'entries', 'errorreportsto', 'etag', 'expired', 'expired_parsed', 'feed',
    'generator', 'generator_detail', 'headers', 'height', 'href', 'icon', 'id',
    'image', 'info', 'info_detail', 'label', 'language', 'length', 'license',
    'link', 'links', 'logo', 'modified', 'name', 'namespaces', 'path', 'port',
    'protocol', 'published', 'published_parsed', 'publisher',
    'publisher_detail', 'registerProcedure', 'rel', 'rights', 'rights_detail',
    'scheme', 'source', 'status', 'subtitle', 'subtitle_detail', 'summary',
    'summary_detail', 'tags', 'term', 'textinput', 'title', 'title_detail',
    'ttl', 'type', 'updated', 'updated_parsed', 'value', 'version', 'width',
]
...
def py2erl(term, root):
    ...
    if isinstance(term, dict) and root in ['namespaces', 'headers']:
        return [
            (py2erl(key, root), py2erl(value, root))
            for key, value in term.items()
        ]

    if isinstance(term, dict):
        return [
            (Atom(key), py2erl(value, key))
            for key, value in term.items()
            if value and key in all_tags
        ]
